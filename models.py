...
class roughSegmenter(nn.Module):
    def __init__(self, n_classes=6, in_channels=3, p_drop=0.5):
        super(roughSegmenter, self).__init__()
        # Input --> (in_channels, 64, 128, 128)
        ...
        self.rc1 = nn.Conv3d(in_channels=128, out_channels=128, kernel_size=3, padding=1)
        self.rc1_bn = nn.BatchNorm3d(128)
        self.rc1_drop = nn.Dropout3d(p=p_drop)
        # conv layer set 4 - up 1
        self.c7 = nn.Conv3d(in_channels=64+128, out_channels=64, kernel_size=3, padding=1)
        self.bn7 = nn.BatchNorm3d(64)
        self.drop7 = nn.Dropout3d(p=p_drop)
        self.c8 = nn.Conv3d(in_channels=64, out_channels=64, kernel_size=3, padding=1)
        self.bn8 = nn.BatchNorm3d(64)
        self.drop8 = nn.Dropout3d(p=p_drop)
        self.sc4 = nn.Conv3d(in_channels=64+128, out_channels=64, kernel_size=(1,1,1))
        self.sc4_bn = nn.BatchNorm3d(64)
        # upsample 2
        self.rc2 = nn.Conv3d(in_channels=64, out_channels=64, kernel_size=3, padding=1)
        self.rc2_bn = nn.BatchNorm3d(64)
        self.rc2_drop = nn.Dropout3d(p=p_drop)
        # conv layer set 5 - up 2
        self.c9 = nn.Conv3d(in_channels=32+64, out_channels=32, kernel_size=3, padding=1)
        self.bn9 = nn.BatchNorm3d(32)
        self.drop9 = nn.Dropout3d(p=p_drop)
        self.c10 = nn.Conv3d(in_channels=32, out_channels=32, kernel_size=3, padding=1)
        self.bn10 = nn.BatchNorm3d(32)
        self.drop10 = nn.Dropout3d(p=p_drop)
        self.sc5 = nn.Conv3d(in_channels=32+64, out_channels=32, kernel_size=(1,1,1))
        self.sc5_bn = nn.BatchNorm3d(32)
    ...
    def forward(self, x):
        ...
        x = self.drop6(x)
        x += F.relu(self.sc3_bn(self.sc3(base)))
        # Upsample 1
...
class roughSegmenter_deeper(nn.Module):
    def __init__(self, n_classes=6, in_channels=3, p_drop=0.5):
        super(roughSegmenter_deeper, self).__init__()
        # Input --> (in_channels, 64, 128, 128)
        # conv layers set 1 - down 1
        self.c1 = nn.Conv3d(in_channels=in_channels, out_channels=32, kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm3d(32)
...
# Checkerboard artifacts from transpose convolutions are very apparent for low depth model
#
class roughSegmenter_TC(nn.Module):
    def __init__(self, filter_factor=2, n_classes=6, in_channels=3, p_drop=0.5):
        super(roughSegmenter_TC, self).__init__()
