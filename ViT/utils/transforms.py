    ...
    def __call__(self, sample):
       #!! Think this assumes channels are in last dim.
        ...
        if random.random() > self.p:
            ...
            pad_width = tuple([(x, x) for x in self.range])
    ...
    def __call__(self, sample):
        if random.random() > self.p:
            ...
            sample["image"] = self.rotation(
                sample["image"], rot_angle=roll_angle, rot_plane=(1, 2), is_mask=False)
            sample["mask"] = self.rotation(
                sample["mask"], rot_angle=roll_angle, rot_plane=(1, 2), is_mask=True)
...
class ScaleImage():
    ...
    def scale(self, image, scale_factor, is_mask):
        ...
        order, cval = (0, 0) if is_mask else (3, -1024)
        height, width, depth = image.shape[:-1]
