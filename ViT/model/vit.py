...
def get_mean_attention_map(attn, token, shape):
    attn = attn[:, :, token, 1:]
    attn = attn.unflatten(2, torch.Size(
        [shape[2] // 16, shape[3] // 16])).float()
        ...
        nn.Unflatten(
            ...
            torch.Size(
                [
                    ...
                    d // pretrained.model.patch_size[2]
                ]
            ),
        )
    )
...
def _resize_pos_embed(self, posemb, gs_h, gs_w, gs_d):
    ...
    posemb_grid = F.interpolate(
        posemb_grid, size=(gs_h, gs_w, gs_d), mode="bilinear")
    posemb_grid = posemb_grid.permute(0, 2, 3, 1).reshape(1, gs_h * gs_w*gs_d, -1)
...
def forward_flex(self, x):
    print('FORWARD_FLEX - vit.py', x.shape)
    b, c, h, w, d = x.shape

    pos_embed = self._resize_pos_embed(
        self.pos_embed, h // self.patch_size[1], w // self.patch_size[0], d//self.patch_size[2]
    )
...
def _make_vit_b16_backbone(
    ...
    pretrained.model.patch_size = [16, 32, 32]
