...
def _crop_to_bounding_box(mask, bbox_min, bbox_max):
    """Crops a 3D mask to the bounding box specified by `bbox_{min,max}`."""
    # we need to zeropad the cropped region with 1 voxel at the lower,
    # the right and the back sides. This is required to obtain the
    # "full" convolution result with the 2x2x2 kernel.
    ...
    # pyformat: disable
    if num_dims == 3:
        cropmask[0:-1, 0:-1, 0:-1] = mask[bbox_min[0]:bbox_max[0] + 1, bbox_min[1]:bbox_max[1] + 1, bbox_min[2]:bbox_max[2] + 1]
    else:
        assert False
    return cropmask
...
def compute_surface_distances(mask_gt, mask_pred, spacing_mm):
    ...
    if num_dims == 3:
        ...
        raise ValueError(f"Only 3D masks are supported, not {num_dims}D.")
...
def compute_average_surface_distance(surface_distances):
    ...
    return (np.abs(average_distance_gt_to_pred)+np.abs(average_distance_pred_to_gt))/2
