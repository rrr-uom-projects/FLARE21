...
from models import ytp_learnableWL
...
def main():
    ...
    # set directories
    checkpoint_dir = "/data/FLARE21/models/ytp_learnableWL/fold"+str(args.fold_num)
    ...
    # Create the model
    n_classes = 7
    model = ytp_learnableWL(n_classes=n_classes, in_channels=3, initial_levels=[50,60,488], initial_windows=[400,100,3024], p_drop=0)

    for num, param in enumerate(model.parameters()):
        if num > 1:
            param.requires_grad = True
        else:
            param.requires_grad = False # only train window-level after a certain number of epochs
    ...
    level_lr, window_lr, default_lr = 0.001, 0.001, 0.005
    optimizer = optim.Adam([{'params': model.WiLe.levels, 'lr':level_lr}, {'params': model.WiLe.windows, 'lr':window_lr}, {'params': [param for num, param in enumerate(model.parameters()) if num > 1]}], lr=default_lr)
